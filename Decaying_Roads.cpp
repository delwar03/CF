#include <bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp>
#include <ext/pb_ds/tree_policy.hpp>
using namespace __gnu_pbds;
template <typename T> using o_set = tree<T, null_type, std::less<T>, rb_tree_tag, tree_order_statistics_node_update>;
#define int long long
#define endl '\n'
#define F first
#define S second
#define pii pair<int, int>
#define sz(x) (int) x.size()
using namespace std;
const int mod = 1e9 + 7;
const int N = 2e5 + 10;
const int INF = 1e18 + 10;
mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());

void solve() {
    int n, m, k; cin>>n>>m>>k;
    int sz = n + m + 5, s = sz - 1, t = sz - 2;
    vector<int> g[sz];
    vector<vector<int>> cap(sz, vector<int>(sz));

    auto addEdge = [&] (int u, int v, int c) {
        g[u].push_back(v);
        g[v].push_back(u);
        cap[u][v] += c;
    };

    for(int i = 1; i <= n; i++) {
        addEdge(s, i, +1);
    }

    vector<pii> v(k);
    for(auto &[x, y] : v) {
        cin>>x>>y;
        addEdge(x, y + n, +1);
    }

    vector<int> cc(m + 1);
    for(int i = 1; i <= m; i++) cin>>cc[i];

    int z; cin>>z;
    vector<pii> que(z);
    for(auto &[x, y] : que) {
        cin>>x>>y;
        cc[x] -= y;
    }

    // for(auto it : cc) cerr<<it<<" "; cerr<<endl;

    for(int i = 1; i <= m; i++) addEdge(i + n, t, cc[i]);

    vector<int> par;
    int flow = 0;

    auto bfs = [&] (int s, int t) {
        par = vector<int>(sz, -1);
        queue<pii> q;
        q.push({s, INF});
        par[s] = 0;
        while(!q.empty()) {
            auto [u, f] = q.front(); q.pop();
            for(auto v : g[u]) {
                if(par[v] == -1 && cap[u][v] > 0) {
                    int ff = min(f, cap[u][v]);
                    par[v] = u;
                    if(v == t) return ff;
                    q.push({v, ff});
                }
            }
        }
        return 0LL;
    };


    auto getFlow = [&] (int s, int t) {
        while(int cur = bfs(s, t)) {
            flow += cur;
            int tmp = t;
            while(tmp != s) {
                int prv = par[tmp];
                cap[prv][tmp] -= cur;
                cap[tmp][prv] += cur;
                tmp = prv;
            }
        }
        return flow;
    };

    reverse(que.begin(), que.end());
    vector<int> ans;
    for(auto [x, y] : que) {
        cap[x + n][t] += y;
        ans.push_back(getFlow(s, t));
    }

    reverse(ans.begin(), ans.end());
    for(auto it : ans) cout<<it<<endl;
}

signed main() {
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);

    int t = 1, c = 1; //cin>>t;
    while(t--) {
        // cerr<<"Case "<<c++<<": \n";
        solve();
    }
}